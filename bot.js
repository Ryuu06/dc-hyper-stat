...
let pub = "847911788452380674"; //  PUBLİC KATAGORİ ID
let kayıt = "872913844085862471"   // KAYIT KATAGORİ ID
let terapi = "884224075306778694"  // TERAPİ KATEGORİ ID
let sorun = "884224075306778694"   // SORUN ÇÖZME KATAGORİ ID
let vk = "VK KATEGORİ ID"   // VAMPİR KÖYLÜ KATAGORİ ID
let dc = "DC KATEGORİ ID"  // DOĞRULUK CESARETLİK KATAGORİ ID
let game = "848220849705320450"  // OYUN KATAGORİ ID
let priv = "851850867178209280"    // PRİVATE KATAGORİ ID
let alone = "882929305070469131"   // ALONE KATAGORİ ID
...
//---------------------------------- BOTU SESLİ SOKMA ----------------------------------------\\
  ...
  client.on("ready", () => {
  client.channels.cache.get("881414534672289833").join();
  });

//---------------------------------- BOTU SESLİ SOKMA ----------------------------------------\\

 client.on("ready", async () => {
client.user.setPresence({ activity: { name: "HyperFamily Sunucumuza Hoşgeldin" }, status: "online" });
})
